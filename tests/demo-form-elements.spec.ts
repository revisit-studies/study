...
test('Test questionnaire component with responses and randomizing questions and responses', async ({ page }) => {
  ...
  const universityCampus = page.getByLabel('Which campus did you attend?');
  await expect(universityCampus).toHaveCount(0);
  await universityName.fill('University of Utah');
  await expect(universityCampus).toBeVisible();
  await universityCampus.fill('Salt Lake City');
  await universityName.fill('Another university');
  await expect(universityCampus).toHaveCount(0);
  await universityName.fill('WPI');
  await expect(universityCampus).toHaveValue('');
  await universityCampus.fill('Worcester');
