...
const filesObj = import.meta.glob('/public/basic-matrices-study/data/*');
const files = Object.keys(filesObj).map((path) => path.replace('/public/basic-matrices-study/data/', ''));
...
export const getData = createAsyncThunk('data/getData', async (file, { rejectWithValue }) => {
  ...
  try {
    const response = await fetch(`/public/basic-matrices-study/data/${file}`);
