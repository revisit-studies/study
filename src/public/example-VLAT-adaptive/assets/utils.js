...
function irtLikelihoodCorrect(a, b, theta) {
  return 1 / (1 + Math.exp(-a * (theta + b)));
}

function irtLikelihoodWrong(a, b, theta) {
  return 1 - irtLikelihoodCorrect(a, b, theta);
}

function itemInfoFunc(a, b, theta) {
  const p = irtLikelihoodCorrect(a, b, theta);
  return a ** 2 * p * (1 - p);
}

function dfToDict(dfList) {
  const newDict = {};
  ...
  return [newDict, unusedChartTaskSet];
}
...
function adaptiveVLATAlt(responseHistoryDict) {
  ...
  const variance = abilityTestTryout.reduce((sum, val) => sum + (val - initialPriorMean) ** 2, 0) / abilityTestTryout.length;
  ...
  const isUsed = (chart, task) => usedChartTaskCombo.has(`${chart}::${task}`);
...
export function getVLATnextqid(qid, correct) {
  const responseHistoryDict = {};
  for (let i = 0; i < qid.length; i += 1) {
    responseHistoryDict[parseInt(qid[i], 10)] = parseInt(correct[i], 10);
  }
