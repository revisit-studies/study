...
export function usePreviousStep() {
  ...
  const goToPreviousStep = useCallback((collectData = true) => {
    ...
    if (hasSkipBlock) {
      ...
      skipConditions.some((condition) => {
        ...
        if (conditionIsTriggered) {
          const previousStepIndex = participantSequence.indexOf(condition.to);
          const previousStepBlockIndex = previousStepIndex === -1 ? findIndexOfBlock(sequence, condition.to) : -1;
          previousStep = previousStepIndex === -1 ? previousStepBlockIndex : previousStepIndex;
          return true;
        }
        return false;
      });
