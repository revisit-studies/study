...
export function useNextStep(responseIdsWithOther: string[] = []) {
  ...
  const goToNextStep = useCallback((collectData = true) => {
    try {
      ...
      const answer = getAnswersFromAllLocations(validation, responseIdsWithOther);
