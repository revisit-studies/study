...
describe('randomizeForm', () => {
  test('returns response IDs in original order when responseOrder is fixed', () => {
    const comp = makeComp([{ id: 'q1' }, { id: 'q2' }, { id: 'q3' }], 'fixed');
  ...
  test('excluded responses stay at their original index when randomized', () => {
    ...
    const comp = makeComp(
      [
        { id: 'q1' },
        { id: 'q2', excludeFromRandomization: true },
        { id: 'q3' },
      ],
      'random',
    );
  ...
  test('all-excluded responses preserve exact order', () => {
    const comp = makeComp(
      [
        { id: 'a', excludeFromRandomization: true },
        { id: 'b', excludeFromRandomization: true },
      ],
      'random',
    );
    expect(randomizeForm(comp)).toEqual({ response: ['a', 'b'] });
...
describe('randomizeOptions', () => {
  ...
  test('random optionOrder returns same options in any order for radio', () => {
    const comp = makeComp([{
      id: 'q1', type: 'radio', options: ['A', 'B', 'C'], optionOrder: 'random',
    }]);
    const result = randomizeOptions(comp);
  ...
  test('processes checkbox responses', () => {
    const comp = makeComp([{
      id: 'c1', type: 'checkbox', options: ['P', 'Q'], optionOrder: 'fixed',
    }]);
    const result = randomizeOptions(comp);
  ...
  test('processes buttons responses', () => {
    const comp = makeComp([{
      id: 'b1', type: 'buttons', options: ['Yes', 'No'], optionOrder: 'fixed',
    }]);
    const result = randomizeOptions(comp);
  ...
  test('processes multiple responses and skips non-option types', () => {
    const comp = makeComp([
      { id: 'text1', type: 'shortText' },
      {
        id: 'r1', type: 'radio', options: ['A', 'B'], optionOrder: 'fixed',
      },
      {
        id: 'c1', type: 'checkbox', options: ['X', 'Y'], optionOrder: 'fixed',
      },
    ]);
    const result = randomizeOptions(comp);
...
describe('randomizeQuestionOrder', () => {
  test('skips non-matrix responses', () => {
    const comp = makeComp([{
      id: 'q1', type: 'radio', options: ['A'], questionOptions: ['Q1'],
    }]);
