...
function makeComp(overrides: Partial<IndividualComponent> & { response: Response[] }): IndividualComponent {
  return {
    type: 'markdown',
    path: '/test.md',
    ...overrides,
...
describe('randomizeForm', () => {
  test('returns response IDs in original order when responseOrder is fixed', () => {
    const comp = makeComp({ response: [radio({ id: 'q1' }), radio({ id: 'q2' }), radio({ id: 'q3' })], responseOrder: 'fixed' });
  ...
  test('returns response IDs in original order when responseOrder is undefined', () => {
    const comp = makeComp({ response: [radio({ id: 'a' }), radio({ id: 'b' })] });
    expect(randomizeForm(comp)).toEqual({ response: ['a', 'b'] });
  });

  test('returns all IDs when responseOrder is random (same set, any order)', () => {
    const comp = makeComp({ response: [radio({ id: 'q1' }), radio({ id: 'q2' }), radio({ id: 'q3' })], responseOrder: 'random' });
...
describe('randomizeOptions', () => {
  test('skips non-radio/checkbox/buttons responses', () => {
    const comp = makeComp({ response: [shortText({ id: 'q1' })] });
    expect(randomizeOptions(comp)).toEqual({});
  });

  test('returns parsed options in order for radio with fixed optionOrder', () => {
    const comp = makeComp({ response: [radio({ id: 'q1', options: ['A', 'B', 'C'], optionOrder: 'fixed' })] });
  ...
  test('returns parsed options in order when optionOrder is undefined', () => {
    const comp = makeComp({ response: [radio({ id: 'q1', options: ['X', 'Y'] })] });
    const result = randomizeOptions(comp);
    expect(result.q1.map((o) => o.value)).toEqual(['X', 'Y']);
  });

  test('random optionOrder returns same options in any order for radio', () => {
    const comp = makeComp({ response: [radio({ id: 'q1', options: ['A', 'B', 'C'], optionOrder: 'random' })] });
    const result = randomizeOptions(comp);
  ...
  test('processes checkbox responses', () => {
    const comp = makeComp({ response: [checkbox({ id: 'c1', options: ['P', 'Q'], optionOrder: 'fixed' })] });
    const result = randomizeOptions(comp);
    expect(result.c1.map((o) => o.value)).toEqual(['P', 'Q']);
  });

  test('processes buttons responses', () => {
    const comp = makeComp({ response: [buttons({ id: 'b1', options: ['Yes', 'No'], optionOrder: 'fixed' })] });
    const result = randomizeOptions(comp);
...
describe('randomizeQuestionOrder', () => {
  test('stores matrix question order by value when rows have side labels', () => {
