...
describe('sanitizeStringForUrl', () => {
  test('keeps semantic version suffix in the slug while normalizing dots to underscores', () => {
    expect(sanitizeStringForUrl('test-config-1.2')).toBe('test-config-1_2');
  });

  test('replaces all periods, spaces, and slashes', () => {
  ...
  test('removes known file extensions', () => {
