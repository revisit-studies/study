...
export function encryptIndex(index: number): string {
  const encrypted = AES.encrypt(index.toString(), key, { mode: CryptoJS.mode.ECB }).toString();
  return btoa(encrypted);
}

export function decryptIndex(index: string): number {
  const decrypted = AES.decrypt(atob(index), key, { mode: CryptoJS.mode.ECB }).toString(enc.Utf8);
