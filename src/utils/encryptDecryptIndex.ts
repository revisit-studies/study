...
export function parseStepIndex(value: string): number | null {
  try {
    const index = decryptIndex(value);
    // Only accept non-negative integers in the format generated by study navigation.
    return Number.isSafeInteger(index) && index >= 0 && encryptIndex(index) === value ? index : null;
  } catch {
    return null;
  }
}
