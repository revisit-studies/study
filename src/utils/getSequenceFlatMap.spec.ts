...
// Sequence helper — for functions that accept the store Sequence type (findBlockForStep, findIndexOfBlock, getSequenceFlatMap)
const seq = (components: (string | Sequence)[], extra: Partial<Sequence> = {}): Sequence => ({
  order: 'fixed',
  orderPath: 'root',
  skip: [],
  components,
...
// ComponentBlock helper — for functions that accept StudyConfig['sequence'] (findFuncBlock, getSequenceFlatMapWithInterruptions)
const cb = (components: ComponentBlock['components'], extra: Partial<ComponentBlock> = {}): ComponentBlock => ({
  order: 'fixed',
  components,
  ...extra,
});
