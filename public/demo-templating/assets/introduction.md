...
In this study, every trial is a "guess the capital" question reusing a single `quizQuestion` base component. Each trial's `parameters` (country, continent, hint file, fun fact) are substituted into:

- the `instruction` text
- the `path` used to pick which hint markdown file to load
- the **content** of that hint markdown file itself
