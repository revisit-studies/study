...
Pull Requests
When asked to make a pull request, target `dev` by default. For issue-driven PRs against `dev`, do not rely on `Closes #...` to create the issue linkage because GitHub only treats closing keywords as closing references when the PR targets the repository default branch. Manually link the PR from the issue's Development section in the GitHub UI. The public GitHub API/`gh` CLI can verify `closingIssuesReferences` and manage linked branches, but it does not expose a reliable mutation for adding an existing PR to an issue's Development section.
